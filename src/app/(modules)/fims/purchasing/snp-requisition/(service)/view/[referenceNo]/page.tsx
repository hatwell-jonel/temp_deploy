...
import { serviceRequisitionViewSearchParams } from "@/validations/searchParams";
...
export default function ViewServiceRequisitionPage({
  params,
  searchParams,
}: Props) {
  const { budgetDetails, print, showButtons } =
    serviceRequisitionViewSearchParams.parse(searchParams);
  return (
    <>
...
export async function ReferenceNumberDetails({
  ...
  if (!requisition)
    return <p>This reference number has no service requisition details!</p>;
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4 p-4 pt-2 text-xs">
        ...
        {showBudgetDetails === true ? (
          <BudgetDetails
            budgetSource={data[0].serviceCategory.budgetSource.name}
            coa={data[0].serviceCategory.chartOfAccounts.name}
            opexCategory={data[0].serviceCategory.opexCategory.category}
            opexType={data[0].serviceCategory.opexCategory.type}
            subAccount={data[0].serviceCategory.subAccounts.name}
          />
        ) : null}
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-md">
        <ApproverForm className="space-y-4" referenceNo={referenceNo}>
          <div>
