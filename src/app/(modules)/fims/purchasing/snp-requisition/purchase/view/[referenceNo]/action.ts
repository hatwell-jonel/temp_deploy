"use server";
import { db } from "@/db";
import { purchasing, requisition } from "@/db/schema/fims";
import { purchase } from "@/db/schema/fims";
...
export async function review(
  ...
  try {
    ...
    revalidatePath("/fims/purchasing/snp-requisition/purchase");
    return {
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx
        .update(purchase)
        .set({ rejectionId: reasonId ?? null })
        .where(eq(purchase.requisitionNo, referenceNo));
    });

    const newStatus = approverStatus === 2 ? "Declined" : "Approved";

    revalidatePath("/fims/purchasing/snp-requisition/purchase", "layout");
    return {
      message: `Successfully changed status to ${newStatus}.`,
      success: true,
    };
