...
import { purchaseRequisitionSearchParams } from "@/validations/searchParams";
...
export default function Page({ searchParams }: PageProps) {
  const parsed = purchaseRequisitionSearchParams.parse(searchParams);
  return (
    <>
      <div className="flex flex-col gap-6 py-8">
        <TitleCard title={title}>
          <Link
            href={"/fims/purchasing/snp-requisition/purchase/create"}
            className={cn(
              buttonVariants({ size: "long" }),
              "w-full text-xs lg:mr-6 lg:w-fit lg:px-4",
            )}
          >
            <Icons.add className="mr-1 size-4" /> Create Purchase Requisition
          </Link>
        </TitleCard>
        <RequisitionTabs />
        <Suspense>
        ...
        <Table className="text-xs">
          <TableHeader className="[&_tr]:border-b-0">
            ...
            <TableRow className="[&_th]:h-8 *:whitespace-nowrap *:text-center *:text-foreground">
              ...
              <TableHead className="border-gray-600 border-r text-center">
              ...
              >
                <TableCell className="whitespace-nowrap p-1.5 text-center">
                  {role !== "requester" ? (
                    <Link
                      ...
                      href={`/fims/purchasing/snp-requisition/purchase/view/${data.requisitionNo}`}
                    >
                      {data.requisitionNo}
                    </Link>
                  ) : (
                ...
                <TableCell className="p-1.5 text-center">
                  <div className="flex items-center justify-center gap-2">
                    <Link
                      href={`/fims/purchasing/snp-requisition/purchase/view/${data.requisitionNo}?showButtons=false`}
                      className="text-xs"
                      title="View"
                    >
                      <Icons.view />
                    ...
                    {data.purchasing.requisitionFinalStatus === 0 &&
                      role === "requester" && (
                        <Link
                          href={`/fims/purchasing/snp-requisition/purchase/edit/${data.requisitionNo}`}
                          className="text-xs"
                          title="Edit"
                        >
                          <Icons.edit />
