...
const basePath = "/fims/purchasing/snp-requisition";

const tabs: Tab[] = [
  { name: "Service", path: basePath, short: "SR" },
  { name: "Recurring - Service", path: `${basePath}/recurring`, short: "SR-R" },
  { name: "Purchase", path: `${basePath}/purchase`, short: "PR" },
];
      ...
      {tabs.map((tab) => (
        <div className="flex items-center justify-center" key={tab.name}>
          ...
          >
            ...
            <div className="block whitespace-nowrap lg:hidden">{tab.short}</div>
          </Link>
        </div>
      ))}
    </div>
