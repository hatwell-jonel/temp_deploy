...
const path = "/fims/purchasing/canvas-request/approval/purchase";
...
export async function decline(
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx
        .update(canvasPurchase)
        .set({ reasonId: reasonId ?? null })
        .where(eq(canvasPurchase.canvasRequestNo, referenceNo));
    });
    revalidatePath(path);
    return {
      message: "Successfully changed status to Declined!",
      success: true,
    };
  } catch (error) {
    console.log({ error });
      ...
      if (finalStatus) {
        ...
        if (finalStatus === 1) {
          ...
          const { canvasRequestNo, requestedBy, createdBy, ...rest } =
            canvassingData;
          const requestNo = await generateCodeFromDate({ prefix: "FLS-PR" });
          ...
          const canvasPurchase = await tx.query.canvasPurchase.findFirst({
            where: (table, { eq }) => eq(table.canvasRequestNo, referenceNo),
            with: {
              itemDescription: {
                with: {
          ...
          if (canvasPurchase?.itemDescription?.itemCategory) {
            const {
              chartOfAccountsId,
              opexCategoryId,
              subAccountsId,
              budgetSourceId,
            } = canvasPurchase.itemDescription.itemCategory;
            const date = new Date();
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      if (unMarkedIds.length > 0) {
        await tx
          .delete(canvasPurchase)
          .where(inArray(canvasPurchase.id, unMarkedIds));
      }
    });
