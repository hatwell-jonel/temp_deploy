  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>Register OpEx Category</DialogTitle>
        </DialogHeader>
          ...
          {Array.from({ length: rows }).map((_, index) => (
            <div className="flex items-center gap-2" key={index}>
              <div className="flex flex-1">
                ...
                <Input name="type" placeholder="Type" className="ml-2" />
              </div>
              {rows === index + 1 ? (
                <Button
                  type="button"
...
export function EditForm({
  ...
  defaultValues: typeof opexCategory.$inferSelect;
}) {
  const [open, setOpen] = React.useState(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>Edit OpEx Category</DialogTitle>
        </DialogHeader>
        <form className="space-y-2" action={action} ref={formRef}>
