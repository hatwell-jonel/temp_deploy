...
const path = "/fims/purchasing/data-mgmt/reasons-for-rejection";
...
export async function create(_: State, formData: FormData): Promise<State> {
  ...
  try {
    await db.insert(reasonForRejection).values(names.map((name) => ({ name })));
    revalidatePath(path);
    return {
      success: true,
      message: "Successfully registered reason for rejection!",
    };
  } catch (error) {
    return {
      success: false,
...
export async function edit(
  ...
  try {
    await db
      ...
      .set({ name })
      .where(eq(reasonForRejection.id, id));
    revalidatePath(path);
    return {
      success: true,
      message: "Successfully updated reason for rejection!",
    };
  } catch (error) {
    return {
      success: false,
...
export async function toggleStatus(id: number) {
  try {
    const data = await db.query.reasonForRejection.findFirst({
      where: eq(reasonForRejection.id, id),
    });
