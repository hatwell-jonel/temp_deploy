...
import type { chartOfAccounts } from "@/db/schema/fims";
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>Register Chart of Accounts</DialogTitle>
        </DialogHeader>
        <form className="space-y-2" action={action} ref={formRef}>
          {Array.from({ length: rows }).map((_, index) => (
            <div className="flex items-center gap-2" key={index}>
              <div className="flex flex-1">
                <Input name="name" placeholder="Chart of Accounts" />
                <Input
                  name="accountCode"
...
export function EditForm({
  ...
  defaultValues: typeof chartOfAccounts.$inferSelect;
}) {
  const [open, setOpen] = React.useState(false);
