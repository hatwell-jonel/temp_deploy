...
import { db } from "@/db";
import { itemCategory } from "@/db/schema/fims";
import { eq } from "drizzle-orm";
import { revalidatePath } from "next/cache";
import { z } from "zod";
import { zfd } from "zod-form-data";

const path = "/fims/purchasing/data-mgmt/item-category";
...
const nameSchema = zfd.text(
  z
    ...
    .min(3, { message: "Item Category must contain atleast 3 character(s)" }),
);
...
export async function create(_: State, formData: FormData): Promise<State> {
  try {
    ...
    console.log({ parsed });
    const toInsert: (typeof itemCategory.$inferInsert)[] = parsed.data.name.map(
      (name, index) => ({
    ...
    await db.insert(itemCategory).values(toInsert);
    revalidatePath(path);
    return { success: true, message: "Successfully registered item category!" };
  } catch (error) {
    console.log({ error });
    return {
      success: false,
...
export async function edit(
  ...
  try {
    ...
    revalidatePath(path);
    return { success: true, message: "Successfully updated item category!" };
  } catch (error) {
    console.log({ error });
    return {
      success: false,
...
export async function toggleStatus(id: number) {
  try {
    const data = await db.query.itemCategory.findFirst({
      where: eq(itemCategory.id, id),
    });
